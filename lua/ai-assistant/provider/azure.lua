...
local function handle_streaming_response(data, callback)
  ...
  -- Azure OpenAI sends "data: [DONE]" to indicate the end of the stream
  ...
  local delta = result.choices and result.choices[1] and result.choices[1].delta
  if not delta or not delta.content then
    return false
  end
...
function M.stream_response(request_id, prompt, context, options, callback)
  ...
  -- Format the endpoint URL with the model
  local model = options.model or M.config.api_model
  local endpoint = M.config.api_endpoint:gsub("{{model}}", model)
...
function M.complete(prompt, context, options)
  ...
  local content = result.choices and result.choices[1] and result.choices[1].message and result.choices[1].message.content
  local finish_reason = result.choices and result.choices[1] and result.choices[1].finish_reason
...
function M.is_available()
  return M.config.api_key and M.config.api_key ~= "" and 
         M.config.api_endpoint and M.config.api_endpoint ~= "" and
         curl_present
end
